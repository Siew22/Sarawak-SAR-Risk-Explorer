...
# 1. 页面配置
st.set_page_config(
    page_title="SRAMS Command Center",
    page_icon="📡",
    layout="wide",
...
<style>
    ...
    iframe { border-radius: 10px; border: 2px solid #374151; }
</style>
            ...
            with details_placeholder:
                ...
                if selected_report['photo_url']:
                    ...
                    if os.path.exists(clean_path):
                        st.image(clean_path, use_column_width='auto')
                    else:
...
if not df.empty:
    ...
    for index, row in df.iterrows():
        ...
        folium.Marker(
            ...
            icon=folium.Icon(color="red" if row['report_type'] == 'road_condition' else "orange", icon="exclamation-triangle", prefix='fa')
        ).add_to(m)
