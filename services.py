...
def geocode_location(location_name: str, proximity_point: Optional[schemas.Point] = None) -> Optional[schemas.Point]:
    ...
    geolocator = Nominatim(user_agent="jalansafe_ai_app/3.2_NAV")
    try:
...
def _fetch_graphhopper_route(points: List[str]) -> Optional[Dict]:
    ...
    # CRITICAL CHANGE: instructions=true to get turn-by-turn data
    params = {
        "point": points, "vehicle": "car", "key": GRAPHHOPPER_API_KEY, 
        ...
        if r.status_code == 200:
            ...
            if data.get('paths'):
                ...
                return {
                    "id": r_id, 
                    "geometry": p['points'], 
                    "base_travel_time": p['time']/1000, 
                    "distance": p['distance'],
                    "instructions": p.get('instructions', []) # Capture instructions
                }
    except: pass
    return None
...
def is_point_near_route_haversine(issue_point: tuple, route_geometry: str, threshold_meters: int = 500) -> bool:
    try:
        ...
        for i in range(0, len(path_points), 5): 
            if haversine(issue_point, path_points[i], unit=Unit.METERS) <= threshold_meters: return True
    except: pass
    return False

def calculate_ai_smart_routes(routes: List[Dict], db: Session) -> List[Dict]:
    if not routes: return []
    print(f"\n--- [AI SCORING] Calculating... ---")
    ...
    for i, route in enumerate(routes):
        ...
        score = 100.0
        score -= len(issues_found) * 12
        score -= user_count * 4
        time_diff = route['base_travel_time'] - optimal_time
        if optimal_time > 0: score -= (time_diff / optimal_time) * 50 
        final_score = max(0, score)
        
        color = 'yellow'
        if final_score > 75: color = 'green'
        elif final_score < 50: color = 'red'
        if is_shortest and len(issues_found) == 0 and user_count < 5: color = 'green'

        # --- 新增规则：如果有问题但没拥堵，强制保持黄色（防止变红）---
        if len(issues_found) > 0 and user_count == 0 and final_score < 50:
             # 如果分数因为问题太多掉到了红色，但其实没有车，我们把它拉回黄色
             # 除非问题真的多到离谱（比如 > 5个）
        ...
        route.update({
            ...
            'instructions': route.get('instructions', []) # Pass instructions to frontend
        })
